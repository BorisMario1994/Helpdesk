...
import { BpbNote, HelpdeskNote } from "../../../generated/prisma";
...
class BpbNoteModel {
  ...
  static createFromType(note: BpbNote) {
    const bpbNote = new BpbNoteModel(note.LineNum, note.Tanggal, note.Username, note.Comment, note.Mentions.split(","));
    return bpbNote;
  };
  ...
  static async generateLatestNumber(nomor: string) {
    const latestNumber = await prisma.bpbNote.aggregate({
      _max: { LineNum: true },
      where: { Nomor: nomor }
    });
