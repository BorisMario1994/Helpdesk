...
class HelpdeskCc {
  ...
  updateCcData = async (nomor: string, note: Notes) => {
    const data = new FormData();
    const allData = { ...this, note: note };
    data.append("json", JSON.stringify(allData));
    if (this.file.size > 0)
      data.append("file", this.file);
    return api.apiInstance.put(`/helpdesk/${nomor}/cc/${this.linenum}/feedback`, data, { headers: { "Content-Type": "multipart/form-data" } });

  };
};

export default HelpdeskCc;
