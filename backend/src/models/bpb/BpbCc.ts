...
import { BpbCc, HelpdeskCc } from "../../../generated/prisma";
...
class BpbCcModel {
  ...
  static createFromType(cc: BpbCc) {
    return new BpbCcModel(cc.LineNum, cc.CC, cc.AC, cc.TanggalAC, cc.PIC, cc.NamaFile);
  };
  ...
  static bulkInsertCc = async (nomor: string, ccList: BpbCcModel[], tx?: Prisma.TransactionClient) => {
    const client = tx ?? prisma;
    return client.bpbCc.createMany({
  ...
  static bulkDeleteCc = async (nomor: string, tx?: Prisma.TransactionClient) => {
    const client = tx ?? prisma;
    return client.bpbCc.deleteMany({
      where: { Nomor: nomor }
    });
  };
};

export default BpbCcModel;
