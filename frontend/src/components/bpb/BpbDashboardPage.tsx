...
import { BpbHeaderLayout } from './BpbHeaderLayout';
...
export function BpbDashboardPage() {
  ...
  const [bpbList, setBpbList] = useState<BpbHeader[]>([]);
  const [filteredBpbList, setFilteredBpbList] = useState<BpbHeader[]>([]);
  ...
  const [filterData, setFilterData] = useState<BpbFilter>({ dari: new Date("dd/MM/yyyy"), hingga: new Date("dd/MM/yyyy"), penerbit: "Select one", sortBy: "Select one" });
  ...
  const Row = ({ index, style }: ListChildComponentProps) => {
    const bpb = filteredBpbList[index];
    ...
    return (
      <div ref={rowRef} style={{ ...style, borderBottom: '1px solid #ccc' }}>
        <BpbHeaderLayout key={bpb.nomor} bpb={bpb} index={index} setSize={setSize} />
      </div>
    );
  };
  ...
  const loadData = async () => {
    try {
      ...
      setBpbList(await BpbHeader.getBpbList(type || "all"));
      setNotificationList(await HelpdeskHeader.getNotificationList()); // ([]);
      setFilterData({ dari: new Date("dd/MM/yyyy"), hingga: new Date("dd/MM/yyyy"), penerbit: "Select one", sortBy: "Select one" });
      setResetFilter(true);
      setIsLoading(false);
    } catch (err: any) {
  ...
  useEffect(() => {
    setFilteredBpbList(getFilteredData());
  }, [bpbList, filterData, searchText]);
  ...
  const getFilteredData = () => {
    return [...bpbList].sort((a, b) => {
      ...
      (searchText.length <= 0 || (bpb.nomor.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.keterangan.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.ref.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.cardCode.toUpperCase().indexOf(searchText.toUpperCase()) > -1)) && (filterData.dari.getFullYear() === 1900 || filterData.dari.toString() === "Invalid Date" || bpb.tanggalTerbit >= filterData.dari) && (filterData.hingga.getFullYear() === 1900 || filterData.hingga.toString() === "Invalid Date" || bpb.tanggalTerbit <= filterData.hingga) && (filterData.penerbit === "Select one" || bpb.dari == filterData.penerbit) 
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="h-full flex flex-col mt-5 space-y-5 md:flex-row md:space-y-0 md:flex-1 md:overflow-hidden md:mb-5 select-none">
        <CategoriesNavigationPanel type="bpb" activeCategory={type || "all"} notifList={notificationList} />
        <div className="relative flex flex-col mt-3 order-last md:h-full md:grow-0 md:basis-5/9 md:order-2 md:mt-0 md:px-5">
          <div className="sticky top-0 space-y-3 px-3 pb-5 bg-white rounded-b-xl md:px-0 md:rounded-b-none md:shadow-none">
            <div className="flex justify-between items-center py-2">
              <p className="text-2xl font-bold">
                {
                  type === "all" ? "All BPB" :
                  type === "created" ? "Created BPB" :
                  type === "unpublished" ? "Unpublished" :
                  type === "approved" ? "Approved BPB" :
                  type === "revision" ? "Revision (BPB)" :
                  type === "rejected" ? "Rejected (BPB)" :
                  type === "waiting-for-approval" ? "Waiting for Approval (BPB)" :
                  type === "waiting-for-review" ? "Waiting for Review (BPB)" :
                  type === "job-registration" ? "Job Registration (BPB)" :
                  type === "done" ? "Done (BPB)" : ""
                }
              </p>
              <div className="flex gap-3">
                    ...
                    <div className="max-w-96 flex flex-col gap-2">
                      {
                        ...
                        notificationList.filter(notif => notif.jumlah > 0 && !["Created Helpdesk", "Waiting for Approval", "Waiting for Review", "Approval Request", "Job Registration"].includes(notif.tipe)).map((notif, index) => 
                          ...
                          <div key={notif.tipe} >
                            <div className="flex justify-between items-center gap-2 px-3 py-2 rounded-md cursor-pointer hover:bg-red-100" onDoubleClick={() => navigate(`/bpb/info/${notif.tipe.split("|")[0]}`)}>
                ...
                <div className="hidden items-center space-x-3 px-3 py-2 bg-red-500 rounded-full shadow shadow-red-200 text-white cursor-pointer md:flex" onClick={() => navigate("/bpb/create")}>
                  <span><FontAwesomeIcon icon={faPlus} /></span>
                  <p className="text-sm">Create new BPB</p>
                </div>
              </div>
            </div>
          ...
          <div ref={listDivRef} className="flex flex-col h-full px-3 pb-15 md:px-0 md:pe-1 md:overflow-y-auto">
            {
              isLoading ?
              ...
              :
              bpbList.length <= 0 || (bpbList.find((bpb) => (searchText.length <= 0 || (searchText.length > 0 && (bpb.nomor.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.keterangan.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.ref.toUpperCase().indexOf(searchText.toUpperCase()) > -1 || bpb.cardCode.toUpperCase().indexOf(searchText.toUpperCase()) > -1))) && (filterData.dari.getFullYear() === 1900 || filterData.dari.toString() === "Invalid Date" || bpb.tanggalTerbit >= filterData.dari) && (filterData.hingga.getFullYear() === 1900 || filterData.hingga.toString() === "Invalid Date" || bpb.tanggalTerbit <= filterData.hingga) && (filterData.penerbit === "Select one" || bpb.dari === filterData.penerbit)) === undefined) ? 
              <div className="flex flex-col mt-10 px-5 space-y-5">
                <span className="self-center"><img className="w-32 h-32" src={emptyBox} alt="EMPTY" /></span>
                <p className="self-center text-sm text-center">{bpbList.length <= 0 ? "There are no BPB registered in this category." : "There are no BPB matched for the given search criterion."}</p>
              </div> 
              : 
              <>
                <div className="flex py-2 bg-blue-100 text-center text-sm font-bold">
                  <div className="flex basis-6/12">
                    <p className="mx-auto">Number</p>
                    <p className="mx-auto">Status</p>
                  </div>
                  <p className="basis-4/12 mx-auto">Recipient</p>
                  <p className="basis-2/12 mx-auto">Published Date</p>
                </div>
                <AutoSizer>
                  {
                    ({ height, width }) => (
                      <VariableSizeList ref={listRef} width={width} height={height} itemCount={filteredBpbList.length} itemSize={getSize} >
                        {Row}
                      </VariableSizeList>
                    )
                  }
        ...
        <FiltersPanel isMobileSize={isMobileSize} setFilter={setFilterData} filterReset={resetFilter} setFilterReset={setResetFilter} usernameList={userList.map((user) => user.username)} bagianTextList={[]} />
          ...
          <PopoverPanel anchor="top end" transition className="flex flex-col origin-bottom mb-2 p-2 space-y-1 z-10 border-2 border-red-200 bg-white rounded-lg shadow-md text-sm  transition duration-200 ease-out data-closed:scale-95 data-closed:opacity-0">
            {
              ...
              notificationList.filter(notif => notif.jumlah > 0 && !["Created Helpdesk", "Waiting for Approval", "Waiting for Review", "Approval Request", "Job Registration"].includes(notif.tipe)).map((notif, index) => 
                notif.tipe.indexOf("|") > -1 ?
                <>
                  <div className="flex justify-between items-center gap-3 px-3 py-2 rounded-md cursor-pointer hover:bg-red-100" onDoubleClick={() => navigate(`/bpb/info/${notif.tipe.split("|")[0]}`)}>
        ...
        <div className="absolute bottom-5 end-5 flex items-center space-x-3 px-3 py-1 bg-red-500 rounded-full shadow shadow-red-200 text-white cursor-pointer md:hidden" onClick={() => navigate("/bpb/create")}>
          <span><FontAwesomeIcon icon={faPlus} /></span>
          <p className="text-sm">Create new BPB</p>
        </div>
      </div>
