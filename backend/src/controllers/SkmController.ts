  ...
  updateCcFeedback: async (nomor: string, data: HelpdeskCcModel, note: HelpdeskNoteModel) => {
    return prisma.$transaction(async (tx) => {
      ...
      if (["REVISION", "REJECT", "REQUESTING REVIEW"].includes(updatedCc.ac)) {
        ...
        const noteLog = new HelpdeskNoteModel(linenum, new Date(), updatedCc.cc, (updatedCc.ac === "REVISION" ? "revision_cc" : updatedCc.ac === "REJECT" ? "rejected_cc" : "requesting_review_cc") + (updatedCc.linenum + 1), mentions);
        await noteLog.insertNote(nomor, tx);
